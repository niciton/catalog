// ====это та что на витрине==== \\

<template>
	<div class="card">
		<a :href="getPath(true)" @click.prevent="$router.push(getPath())" class="card_photo">
			<img v-if="cardData.Image" srcset="/icons/no_photo.svg" :data-srcset="cardData.Image"
				:alt="`${cardData.Articul} ${cardData.Name} 225x150`" />
			<img v-else srcset="/icons/no_photo.svg" alt />
		</a>
		<div class="card_body">
			<a :href="getPath(true)" @click.prevent="$router.push(getPath())" class="card_title">
				{{ cardData.Name }}
			</a>
			...
			<div class="card_item card_manufacturer">
				<div class="item_title manufacturer_title">Производитель</div>
				<span class="dots"></span>
				<div class="item_value manufacturer_value">{{ cardData.Manufacturer }}</div>
			</div>
...
export default {
	...
	methods: {
		...
		getPath(isObg = false) {
			const path = getPath('product', this.cardData.Articul, this.cardData.Name)
			// console.log(path);
			if (isObg) {
	...
	mounted() {
		lozad('.card img[data-srcset]').observe()
	},
}
</script>

<style src="@/css/components/card.scss"></style>